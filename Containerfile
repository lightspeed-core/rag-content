...
RUN if [ -f /cachi2/cachi2.env ]; then \
    ...
    gcc cmake cargo rust git libpq-devel swig autoconf automake libtool libxml2-devel libxslt-devel && \
    ${BUILDER_DNF_COMMAND} clean all; \
    fi
...
RUN if [ -f /cachi2/cachi2.env ]; then \
    ...
    uv pip install --no-cache --no-index --find-links ${PIP_FIND_LINKS} --no-deps . && \
    uv pip check; \
    else \
