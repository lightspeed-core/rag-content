...
LIGHTSPEED_STACK_IMAGE="${LIGHTSPEED_STACK_IMAGE:-registry.redhat.io/lightspeed-core/lightspeed-stack-rhel9:v0.6.0rc2}"
...
if ! podman unshare cat /proc/self/uid_map >/dev/null 2>&1; then
  progress "User namespace mapping unavailable, using --userns=host"
  PODMAN_EXTRA_ARGS+=(--userns=host)
fi
...
podman run --rm "${PODMAN_EXTRA_ARGS[@]}" \
...
podman run -d --name lightspeed-stack-e2e \
  --network=host "${PODMAN_EXTRA_ARGS[@]}" \
...
progress "Query returned HTTP $QUERY_HTTP_CODE"
progress "Response (first 500 chars):"
printf '%.500s\n' "$RESPONSE"
