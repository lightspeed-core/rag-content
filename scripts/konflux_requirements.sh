...
# Script to split requirements by index source (CPU or CUDA Hermeto profiles).
#
# Policy:
...
#
# Usage: scripts/konflux_requirements.sh [cpu|cuda]   (default: cpu)
#
...
cpu)
	...
	TEKTON_PREFETCH_FILES=(
		...
		.tekton/rag-content-cpu-0-5-pull-request.yaml
		.tekton/rag-content-cpu-0-5-push.yaml
	)
	;;
*)
...
GEN_TAG="konflux_requirements.sh ($PROFILE)"
...
if grep -qE '^[a-zA-Z0-9][a-zA-Z0-9_.-]*==' "$WHEEL_FILE"; then
	...
	printf '%s\n' "# No RHOAI-only wheel pins; all other wheels use PyPI hashes in $(basename "$WHEEL_HASH_FILE_PYPI")." > "$WHEEL_HASH_FILE"
...
# appears in both, pip must see PyPI hashes (prefetched wheel). Drop the RHOAI copy.
...
# hermetic source file — Hermeto cannot satisfy them (e.g. nvidia-* has no usable sdist for fetch-deps).
