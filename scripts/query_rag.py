...
def _llama_stack_query(args: argparse.Namespace) -> None:
    ...
    cfg = yaml.safe_load(open(os.path.join(args.db_path, "llama-stack.yaml"), "r"))
    db_filename = os.path.basename(
        cfg["providers"]["vector_io"][0]["config"]["db_path"]
    )

    cfg["providers"]["vector_io"][0]["config"]["db_path"] = os.path.realpath(
        os.path.join(args.db_path, db_filename)
    )
    if args.model_path:
    ...
    if len(md["chunks"]) == 0:
        ...
        exit(1)
    if args.threshold > 0.0 and md["scores"][0].score < args.threshold:
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vector-store-type",
        default="auto",
        choices=["auto", "faiss", "llamastack-faiss", "llamastack-sqlitevec"],
    ...
    if vector_store_type == "auto":
        ...
        elif os.path.exists(os.path.join(args.db_path, "sqlitevec_store.db")):
            vector_store_type = "llamastack-sqlitevec"
