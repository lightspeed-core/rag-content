...
$CONTAINER_RUNTIME exec "$CONTAINER_NAME" subscription-manager repos --list-enabled

#echo "Listing ruby-libs"
#$CONTAINER_RUNTIME exec "$CONTAINER_NAME" dnf repoquery --repo=rhel-9-for-x86_64-appstream-eus-rpms ruby-libs --latest-limit=1

echo "Disabling rhel-9-for-x86_64-appstream-eus-rpms repo"
#$CONTAINER_RUNTIME exec "$CONTAINER_NAME" subscription-manager repos --disable=rhel-9-for-x86_64-appstream-eus-rpms --disable=codeready-builder-for-rhel-9-x86_64-eus-rpms
...
# Follow the same lookup order as containers-auth.json(5): an explicit
# REGISTRY_AUTH_FILE override, the XDG runtime dir (Linux), then the
# per-user fallback under $HOME (used on macOS, where /run/user doesn't
# exist), and finally a plain docker config as a last resort.
AUTH_JSON=""
for candidate in \
    "$REGISTRY_AUTH_FILE" \
...
if [[ -n "$AUTH_JSON" ]]; then
    ...
    echo "Warning: no registry auth file found; private images may fail to pull."
fi
