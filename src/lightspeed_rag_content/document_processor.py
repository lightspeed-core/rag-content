...
class _BaseDB:
    ...
    @classmethod
    def _filter_out_invalid_nodes(cls, nodes: list[TextNode]) -> list[TextNode]:
...
class _LlamaStackDB(_BaseDB):
    ...
    VECTOR_IO_FAISS = """    - provider_id: {provider_id}
      ...
      config:
        kvstore:
          ...
          namespace: null
          db_path: {db_path}"""
    VECTOR_IO_SQLITE_VEC = """    - provider_id: {provider_id}
...
"""
    ...
    def __init__(self, config: _Config):
        ...
        self.tmp_dir: Any = None

    def write_yaml_config(self, provider_id: str, filename: str, db_file: str) -> None:
        ...
        vector_io = getattr(self, "VECTOR_IO_" + template).format(
            db_path=db_file, provider_id=provider_id
        )
    ...
    def _start_llama_stack(self, cfg_file: str) -> Any:
        ...
        """
        # TODO: This part will need to be changed to support multiple usages
    ...
    def save(
        ...
        # We don't pass:
        # - provider_id because there's only 1 provider so it's the default
        # - provider_vector_db_id because we want to use the value from vector_db_id
...
class DocumentProcessor:
    ...
    def save(
        self, index: Optional[str] = None, output_dir: Optional[str] = None
    ) -> None:
